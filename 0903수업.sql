...
-- 보통, 일반 사용자에게 노출하고 싶지 않은 정보들이나,
-- 업무에 필요한 정보들만 출력하고자 할 때 사용한다.
...
-- 뷰는 SELECT 쿼리문을 저장한 논리적인 가상 테이블이기 때문에
...
-- 그 결과를 V_RESULTSET_EMP 라는 뷰를 생성하여 
-- 뷰를 통해 조회하는 뷰 객체를 만들어 보시오.
...
AS SELECT EMP_ID, EMP_NAME, JOB_CODE, 
       DECODE(SUBSTR(EMP_NO,8,1),1,'남','여'),
       EXTRACT (YEAR FROM SYSDATE) - EXTRACT(YEAR FROM HIRE_DATE)
FROM EMPLOYEE;
...
-- 4. JOIN을 이용한 여러 테이블을 참조하는 경우 
      ...
      -- 일반적으로는 불가능
-- 5. DISTINCT를 사용했을 경우 
--    삽입,수정,삭제의 대상이 명확하지 않기 때문에 DML사용이 불가능하다.
-- 6. 그룹 함수를 사용하거나, GROUP BY를 통한 결과일 경우
--    삽입,수정,삭제의 대상이 명확하지 않기 때문에 DML사용이 불가능하다.
...
-- FORCE / NOFORCE : 서브 쿼리에 사용된 테이블이 존재하지 않아도 뷰를 생성할 수 있는 옵션
-- WITH CHECK : 옵션을 설정한 컬럼의 값을 변경하지 못하게 막는 옵션
-- FORCE : 존재하지 않는 테이블이라도 뷰를 강제로 생성 시키는 옵션
...
INSERT INTO V_EMP
VALUES (802,'민경운','101010-1234567','mingh@kh.or.kr',
...
-- SEQUENCE (시퀀스) :
-- 1,2,3,...9,10 의 형식으로 
-- 숫자 데이터를 연속처리 하기 위한 객체 
-- 자동 번호 발생기
-- 순차적으로 정수값을 생성해 주는 객체 (+/-)

/*
CREATE SEQUENCE 시퀀스이름 
[INCREMENT BY 숫자] : 다음 값에 대한 증감 수치, 생략할 경우 1씩 증가한다.
-- INCREMENT BY  5 --> 5씩 증가
-- INCREMENT BY -5 --> 5씩 감소 
[START WITH 숫자] : 처음 시작될 숫자를 지정한다. 생략하면 1부터 시작
[MAX VALUE 숫자 | NOMAXVALUE] -- 발생시킬 최대값 설정(10^27 - 1)
[MIN VALUE 숫자 | NOMINVALUE] -- 발생시킬 최소값 설정(-10^26)
...
-- 시퀀스를 처음 생성했을 경우 반드시 한번은 동작 시켜야 한다.
...
SELECT SEQ_EMPID.NEXTVAL FROM DUAL; -- 최대값 310이므로
...
-- INSERT 구문에서 VALUES를 통한 값 입력 시 사용할 수 있고,
-- UPDATE 구문에서도 SET을 통한 값 변경 시 사용할 수 있다.
-- 단, 서브쿼리의 SELECT 구문에서는 사용이 불가능하다.
-- VIEW 객체에서도 SELECT 시에 사용이 불가능하다.
-- DISTINCT 를 선언할 시 사용할 수 없다.
...
VALUES (SEQ_EID.NEXTVAL,'홍길동','101010-1234567','hong123@kh.or.kr',
        '01012344321','D2','J7','S1',5000000,0.1,200,SYSDATE,NULL,DEFAULT);

SELECT * FROM EMPLOYEE
WHERE EMP_NAME = '홍길동';

-- D9 부서 J7 직급의 직원 4명 더 추가하기
INSERT INTO EMPLOYEE
VALUES (SEQ_EID.NEXTVAL,'공길동','101030-1234567','Kong123@kh.or.kr',
        '01012344321','D2','J7','S1',5000000,0.1,200,SYSDATE,NULL,DEFAULT);
        
INSERT INTO EMPLOYEE
VALUES (SEQ_EID.NEXTVAL,'김길동','101010-1234568','kim123@kh.or.kr',
        '01012344323','D2','J7','S1',5000000,0.1,200,SYSDATE,NULL,DEFAULT);
        
INSERT INTO EMPLOYEE
VALUES (SEQ_EID.NEXTVAL,'이길동','101010-1334567','lee123@kh.or.kr',
        '01032344321','D2','J7','S1',5000000,0.1,200,SYSDATE,NULL,DEFAULT);
        
INSERT INTO EMPLOYEE
VALUES (SEQ_EID.NEXTVAL,'박길동','101110-1234567','park123@kh.or.kr',
        '01012344521','D2','J7','S1',5000000,0.1,200,SYSDATE,NULL,DEFAULT);
...
SELECT SEQ_CYCLE.NEXTVAL FROM DUAL; --최대값 도달 시 최소값부터 시작 
-- CYCLE을 설정했을 경우 최대값 도달 시 최소값부터 시작
...
--         미리 한번에 연산을 수행하여 연산한 결과들을 메모리에 담아 놓았다가 사용하는 방식 
-- EX) 빵을 미리 구워서 파는 가게 
...
SELECT SEQ_CACHE.CURRVAL, INCREMENT_BY, SEQUENCE_NAME, CACHE_SIZE, LAST_NUMBER 
FROM USER_SEQUENCES
...
SELECT SEQ_CACHE.NEXTVAL FROM DUAL;
SELECT SEQ_CACHE.NEXTVAL FROM DUAL;
SELECT SEQ_CACHE.NEXTVAL FROM DUAL; -- LAST_NUMBER에 도착했을 때
...
-- 계산을 다시 수행해야 할 경우 이미 계산 해 두었던 값을 버리고
-- 마지막 부분에서 새로 시작한다.
